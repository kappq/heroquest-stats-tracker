...
@game.route("/create-hero", methods=["GET", "POST"])
@login_required
def create_hero():
    if request.method == "POST":
        ...
        return redirect(url_for("game.hero", hero_id=hero.id))

    return render_template("game/create_hero.html")
...
def heroes():
    ...
    return render_template("game/heroes.html", heroes=heroes)


@game.route("/hero/<int:hero_id>")
@login_required
def hero(hero_id):
    hero = Hero.query.get(hero_id)
    if not hero:
        flash("Hero not found", "error")
        return redirect(url_for("game.heroes"))

    return render_template("game/hero.html", hero=hero)
